    ...
    public partial class FormAddNewGood : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                tableName = tableName.Replace('/', '_');
                int x = aukcionDBDataSet.Tables.IndexOf(tableName);
                dataGridView2.DataSource = aukcionDBDataSet.Tables[x];
                // Подготовка первого запроса
                sqlDataAdapter1.InsertCommand =
                    new SqlCommand(
                        "INSERT INTO Изделие (Название, Хозяин, Тип_Изделия) VALUES (@Название, @Хозяин, @Тип_Изделия) SET @Новый_ID=SCOPE_IDENTITY();",
                        sqlConnection1);
                ...
                for (int i = 1; i < mx; ++i)
                {
                    if (dataGridView2.Rows[0].Cells[i].ValueType.Equals(Type.GetType("String")))
